...
def get_season_info(date_obj):
    ...
    # We want to save to q2_logs specifically
    season_dir = cfg.DATA_DIR / season_id / "q2_logs"
...
def get_valid_game_dates():
    ...
    # Check Master Files (Fastest)
    master_files = sorted(cfg.DATA_DIR.glob(cfg.MASTER_BOX_SCORES_PATTERN))
    
    if not master_files:
        logging.error("No box score files found! Run run_scrape.py first.")
        return []

    logging.info(f"Scanning {len(master_files)} files to find valid game dates...")
...
def main():
    common.setup_logging(name="backfill_quarters")
    logging.info(">>> STARTING Q2 BACKFILL (For 1H Props) <<<")
    
    # 1. Get Exact Game Dates
    target_dates = get_valid_game_dates()
    
    if not target_dates:
        logging.critical("No game dates found. Cannot backfill.")
        return

    logging.info(f"Found {len(target_dates)} active game days to backfill Q2 data.")
    ...
    for i, date_str in enumerate(target_dates):
        ...
        # Get Folder (e.g., prop_data/2025-26/q2_logs)
        season_id, output_dir = get_season_info(dt_obj)
        output_dir.mkdir(parents=True, exist_ok=True)
        
        filename = f"daily_q2_stats_{date_str}"
        file_path = output_dir / f"{filename}.parquet"
        
        # SKIP if already exists
        if file_path.exists():
            # print(f"[{i+1}/{len(target_dates)}] Skipping {date_str} (Exists)")
            continue
            
        logging.info(f"[{i+1}/{len(target_dates)}] Fetching Q2 for {date_str} (Season: {season_id})...")
        
        try:
            # API Call for Period 2 (Q2)
            q2_stats = leaguedashplayerstats.LeagueDashPlayerStats(
                period=2, 
            ...
            df = q2_stats.get_data_frames()[0]
            
            if not df.empty:
                df['GAME_DATE'] = date_str
                scraper.save_clean_parquet(df, filename, output_dir)
            else:
                logging.warning(f"No Q2 stats returned for {date_str}.")
    ...
    logging.info("<<< BACKFILL COMPLETE. NOW UPDATE ETL.PY >>>")
