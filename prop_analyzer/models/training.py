...
def train_single_prop(df, prop_cat):
    ...
    # --- SAMPLE WEIGHT CALCULATION (NEW) ---
    # Give higher weight to the current season (1.0) and lower to past seasons (0.6)
    if 'SEASON_ID' in df.columns:
        latest_season = df['SEASON_ID'].max()
        # Create weights series aligned with df index
        sample_weights = df['SEASON_ID'].apply(lambda x: 1.0 if x == latest_season else 0.6)
        logging.info(f"[{prop_cat}] Applying Sample Weights: Current({latest_season})=1.0, Past=0.6")
    else:
        sample_weights = pd.Series(1.0, index=df.index)
        logging.warning(f"[{prop_cat}] SEASON_ID missing. Defaulting to equal weights (1.0).")
    ...
    # Note: We split weights here too
    ...
    # We generally don't weight validation data for metrics, 
    # but some fit methods accept it for eval_set. We'll stick to weighting training only.
